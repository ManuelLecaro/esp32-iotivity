          ...
          oc_resource_properties_t bm, void *user_data)
{
  ...
  for (i = 0; i < (int)oc_string_array_get_allocated_size(types); i++) {
    ...
    if (strlen(t) == 11 && strncmp(t, "oic.r.light", 11) == 0) {
      ...
      if(oc_do_get(light_1, light_server, NULL, &get_light, LOW_QOS, NULL)){
...
void 
oc_get_resource(const char *uri,oc_endpoint_t *endpoint, const char *query,
          oc_response_handler_t handler, oc_qos_t qos, void *user_data){
    if(oc_do_get(uri, endpoint, query, handler, qos, user_data)){
...
static int client_main(void)
{
  ...
  static const oc_handler_t handler = {.init = app_init,
                                       ...
                                       .requests_entry = issue_requests };//Part That manage the oc_events
...
void app_main(void)
{
    ...
    if (ret == ESP_ERR_NVS_NO_FREE_PAGES || ret != ESP_OK) {
